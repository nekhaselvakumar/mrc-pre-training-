...
function ViewTask() {
  ...
  useEffect(() => {
    console.log("http://localhost:4000/tasks/" + sub.projectid);
    fetch("http://localhost:4000/tasks/" + sub.projectid)
  ...
  //     fetch("http://localhost:4000/users/" + emp_email)
  //       //.then((response) => response.json())
  //       .then((data) => {
  //         //user_row = "efg";
  ...
  function TaskList() {
    ...
    return tasks_arr.map((currenttask) => {
      ...
      fetch("http://localhost:4000/users/" + currenttask.emp_email)
        .then((response) => response.json())
        .then((data) => {
          //setUser_row(data);
          //user_row = data;
